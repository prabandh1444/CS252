 ...
 *      ./waf --run "nsta-udp" (or ./ns3 run "nsta-udp")
 *
 *    To run from cli, with 5 STAs and 1 AP:
 *     ./waf --run "nsta-udp --uplinks=5" (or ./ns3 run "nsta-udp --uplinks=5")
 *
 *    To get more info about the command line options:
 *      ./waf --run "nsta-udp --help" (or ./ns3 run "nsta-udp --help")
 *
...
int main(int argc, char *argv[]) {
  ...
  std::string phyRate = "OfdmRate48Mbps"; // Physical layer bitrate.
  double simulationTime = 3;              // Simulation time in seconds.
  int stepsTime = 1; // Time steps for throughput calculation
  std::string filename = "/tmp/csma-ca-nsta-udp.txt"; // Output filename
  ...
  wifiPhy.EnablePcap("AP", apDevice);
  wifiPhy.EnablePcap("Cli", staDevices);
